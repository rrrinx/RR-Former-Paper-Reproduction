...
def main():
    ...
    for EPOCH in [30, 50, 100]:
        ...
        for idx, (area_id, basin_id) in pbar:
            ...
            state_dict = torch.load(config['pretraining_path'])
            model.load_state_dict(state_dict)

            # print(sum(p.numel() for p in model.parameters()) / 1e6, 'M parameters')
            trainer = Trainer(model, train_loader, val_loader, test_loader, basin_id, area_id, config['train'], EPOCH)
            ...
            state_dict = torch.load(config['train']['save_dir'] + f'/{EPOCH}/{area_id}/{basin_id}_fine_tuning_model.pth')
            model.load_state_dict(state_dict)
        ...
        res_path = config['res_path'] + f'/{EPOCH}'
